...
get_num_threads_per_sm(int block_m)
{
...
__global__ void
__launch_bounds__(get_num_threads_per_sm<kNumLoaderThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
                    ...
                    __nv_fp8_e4m3 *gmem_a, __nv_fp8_e4m3 *gmem_b, float *scales_a)
{
    ...
    smem_scales_b = reinterpret_cast<float *>(smem_buffer + SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE) + kNumStages * SMEM_SCALES_A_SIZE_PER_STAGE);
...
class Gemm
{
  public:
    ...
    static void
    run(__nv_bfloat16 *gmem_d, float *scales_b, int *grouped_layout,
        uint32_t shape_m,

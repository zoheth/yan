...
struct CudaLaunchPolicy {
    ...
    static cudaError_t launchEx(Args&&... args) {
        return cudaLaunchKernelEx(std::forward<Args>(args)...);
    }

    template<typename... Args>
    static cudaError_t launch(Args&&... args) {
        return cudaLaunchKernel(std::forward<Args>(args)...);
    }
};
...
struct NvshmemLaunchPolicy {
    ...
    template<typename... Args>
    static cudaError_t launch(Args&&... args) {
        return nvshmemx_collective_launch(std::forward<Args>(args)...);
    }
};
  ...
  DISPATCH_GQA_GROUP_SIZE(num_qo_heads / num_kv_heads, GROUP_SIZE, {
    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
      ...
      if (tmp_v == nullptr) {
        ...
        if (enable_pdl) {
          FLASHINFER_CUDA_CALL(LaunchPolicy::launchEx(&config, kernel, params));
        } else {
